...
namespace Vezir
{
    public partial class Folyofolyo : Szulogyerekvaltozasok
    {
        ...
        public Folyofolyo(FakUserInterface fak, VezerloControl hivo, Vezerloinfo aktivvezerles)
        {
            InitializeComponent();
        ...
        public override void Button_Click(object sender, EventArgs e)
        {
            ...
            if (conttip != null)
            {
                ...
                if (butname == "ok")
                {
                    i = conttip.MezoControlInfo.InputelemArray.IndexOf((MezoTag)folyoszamlaba.Tag);
                    conttip.MezoControlInfo.InputelemArray[i].SetHibaSzov("");
                    if (!Hibavizsg(UserControlInfo.LastEnter) && !Hibavizsg(conttip))
                    {

                        if (folyoszamlabol.Text == folyoszamlaba.Text)
                        {
                            conttip.MezoControlInfo.InputelemArray[i].SetHibaSzov("Nem lehet azonos!");
                            return;
                        }
                    }
                }
