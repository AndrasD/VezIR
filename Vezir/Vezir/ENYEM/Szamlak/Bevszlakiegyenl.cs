...
namespace Vezir
{
    ...
    public partial class Bevszlakiegyenl : Csakgrid
    {
        ...
        public Bevszlakiegyenl(FakUserInterface fak,VezerloControl hivo, Vezerloinfo vezerles)
        {
            InitializeComponent();
            vezhivo = hivo;
            vezerles.Hivo = hivo;
            ...
            Tabinfo = UserParamTabla.TermCegPluszCegalattiTablainfok["BEVSZLA"];
        ...
        public override void AltalanosInit()
        {
            ...
            if (valt || rogzitett)
            {
                ...
                Sqlinterface.Select(Tabinfo.Adattabla, FakUserInterface.AktualCegconn, "BEVSZLA", " where FIZETVE = 'N' AND MARADEK = 0", " order by SZLA_DATUM", false);
                ValtozasBeallit();
                ...
                else
                {
                    ...
                    if (dataGridView1.Columns.Count == 0)
                    {
                        ...
                        dataGridView1.Columns.Add(Tabinfo.Ujtextcolumn(Tabinfo.TablaColumns["OSSZBEVETEL"], true));
        ...
        public override void rogzit_Click(object sender, EventArgs e)
        {
            dateTimePicker1.Enabled = true;
            Datumtol = dateTimePicker1.Value;
            string sel = " UPDATE BEVSZLA set FIZETVE = 'I', KIEGYENL_DATUM = '" + DatumtolString + "'";
            string felt = " where ";
            bool elso = true;
            for (int i = 0; i < Tabinfo.Adattabla.Rows.Count; i++)
            {
                ...
                if (dr["FIZETVE"].ToString() == "I")
                {
                    ...
                    if (!elso)
                        ...
                        elso = false;
                    felt += "BEVSZLA_ID =" + dr["BEVSZLA_ID"].ToString();
                }
            }
            if (!elso)
            {
                Sqlinterface.SpecCommand(Tabinfo.Adattabla, FakUserInterface.AktualCegconn, "BEVSZLA", sel + felt, "");
                Sqlinterface.Select(Tabinfo.Adattabla, FakUserInterface.AktualCegconn, "BEVSZLA", " where FIZETVE = 'N' AND MARADEK = 0", " order by SZLA_DATUM", true);
                if (Tabinfo.Adattabla.Rows.Count == 0)
                    FakPlusz.MessageBox.Show("Minden számla ki van egyenlitve!");
                Hivo.RogzitesUtan();
                if (this.Visible)
